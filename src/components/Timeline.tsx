    ...
    <section id="timeline" className="py-20 relative overflow-hidden bg-gradient-to-b from-gray-900 to-gray-800 text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <Clock className="text-4xl md:text-6xl text-blue-500 mr-3 stroke-[1.5]" />
          <h2 className="text-2xl md:text-5xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-teal-400">
        ...
        <div className="relative">
          ...
          <motion.div 
            className="absolute left-4 md:left-1/2 top-0 bottom-0 transform md:-translate-x-1/2 w-1 h-full bg-gradient-to-b from-blue-500 via-teal-500 to-blue-500 rounded-full shadow-[0_0_10px_rgba(59,130,246,0.5)]"
          ...
          <div className="space-y-8">
            {timelineData.map((item, index) => (
              <motion.div
                ...
                className={`flex md:items-center ${index % 2 === 0 ? 'flex-col md:flex-row' : 'flex-col md:flex-row-reverse'}`}
              >
                {/* Icon - with animated glow effect */}
                <motion.div
                  whileHover={{ rotate: 360, boxShadow: "0 0 15px rgba(59,130,246,0.8)" }}
                  transition={{ duration: 0.6 }}
                  className="w-12 h-12 md:w-16 md:h-16 rounded-full bg-gradient-to-r from-blue-600 to-teal-600 flex items-center justify-center z-10 shadow-[0_0_10px_rgba(59,130,246,0.5)] absolute left-0 md:static md:left-auto ml-[-1.5rem] md:ml-0"
                >
                  {item.icon === 'work' ? (
                ...
                <div className="w-full md:w-5/12 pl-12 md:pl-0 md:pr-0">
                  <motion.div
                    ...
                    className={`backdrop-blur-sm bg-white/10 p-4 md:p-6 rounded-xl border border-blue-500/30 shadow-lg text-left md:${index % 2 === 0 ? 'text-right mr-8' : 'text-left ml-8'}`}
                  >
                    <motion.span 
                    ...
                    <p className="text-gray-300 mt-2 text-sm md:text-base">{item.description}</p>
                    
                    {/* Decorative corner accent */}
                    <div className={`absolute top-0 ${index % 2 === 0 ? 'left-0 md:right-0 md:left-auto' : 'left-0'} w-16 h-16 bg-gradient-to-br from-blue-500/20 to-transparent rounded-br-full`}></div>
                  </motion.div>
                </div>
